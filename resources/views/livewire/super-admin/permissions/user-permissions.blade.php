<div class="max-w-4xl mx-auto py-8 px-4 space-y-6">
    ...
    <div class="flex items-center gap-3">
        <a href="{{ route('superadmin.users.index') }}" class="text-neutral-400 hover:text-neutral-600 dark:hover:text-neutral-200">
            ← Voltar
        </a>
        <div>
    ...
    @if($companies->isEmpty())
        <div class="bg-white border rounded-xl shadow-sm p-8 text-center dark:bg-zinc-800 dark:border-zinc-700">
            <p class="text-neutral-500 dark:text-neutral-400">Este usuário não está vinculado a nenhuma empresa.</p>
        </div>
    @else
                        ...
                        @foreach($permissions as $group => $groupPermissions)
                            ...
                            @foreach($groupPermissions as $permission)
                                @php
                                    ...
                                    $permissionKey = str_replace('.', '__', $permission->name);
                                    $override  = $overrides[$permissionKey] ?? 'default';
                                @endphp
                                <tr class="hover:bg-neutral-50 dark:hover:bg-zinc-700/30">
                                    ...
                                    <td class="py-2.5 text-center">
                                        <flux:select
                                            wire:model="overrides.{{ $permissionKey }}"
                                        >
