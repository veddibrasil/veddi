<div class="w-full space-y-6"
     x-init="
         ...
         $watch(() => $wire.confirmingPdvActivation, val => val ? $flux.modal('confirm-pdv-activation').show() : $flux.modal('confirm-pdv-activation').close());
         $watch(() => $wire.confirmingPdvCancellation, val => val ? $flux.modal('confirm-pdv-cancellation').show() : $flux.modal('confirm-pdv-cancellation').close());
         $watch(() => $wire.confirmingFiscalActivation, val => val ? $flux.modal('confirm-fiscal-activation').show() : $flux.modal('confirm-fiscal-activation').close());
         $watch(() => $wire.confirmingFiscalCancellation, val => val ? $flux.modal('confirm-fiscal-cancellation').show() : $flux.modal('confirm-fiscal-cancellation').close());
         $watch(() => $wire.confirmingWaiterActivation, val => val ? $flux.modal('confirm-waiter-activation').show() : $flux.modal('confirm-waiter-activation').close());
         $watch(() => $wire.confirmingWaiterCancellation, val => val ? $flux.modal('confirm-waiter-cancellation').show() : $flux.modal('confirm-waiter-cancellation').close());
     "
     x-on:open-plan-card-modal.window="$flux.modal('plan-change-card').show()"
     x-on:open-pdv-card-modal.window="$flux.modal('pdv-module-card').show()"
     x-on:open-fiscal-card-modal.window="$flux.modal('fiscal-module-card').show()"
     x-on:open-waiter-card-modal.window="$flux.modal('waiter-module-card').show()">
    ...
    {{-- Módulo PDV (addon independente do plano) --}}
    <div class="bg-white border rounded-xl shadow-sm p-6 space-y-4 dark:bg-zinc-800 dark:border-zinc-700">
        <div class="flex items-center justify-between flex-wrap gap-4">
            <div class="space-y-1">
                ...
                <p class="text-sm text-neutral-500 dark:text-neutral-400">
                    Terminal de venda presencial, disponível para qualquer plano. Cobrado junto com a assinatura do seu plano, em uma única fatura.
                </p>
            </div>
                ...
                @else
                    <div class="text-right">
                        <span class="text-xl font-bold text-neutral-800 dark:text-neutral-100">
                            R$ {{ number_format($pdvAddonAmount, 2, ',', '.') }}
                        </span>
                        <span class="text-sm text-neutral-500 dark:text-neutral-400">/mês</span>
                    </div>
                    <button wire:click="confirmPdvActivation"
    ...
    {{-- Módulo Fiscal (addon independente do plano) --}}
    <div class="bg-white border rounded-xl shadow-sm p-6 space-y-4 dark:bg-zinc-800 dark:border-zinc-700">
        <div class="flex items-center justify-between flex-wrap gap-4">
            <div class="space-y-1">
                <h2 class="font-semibold text-neutral-700 text-sm uppercase tracking-wide dark:text-neutral-300">
                ...
                <p class="text-sm text-neutral-500 dark:text-neutral-400">
                    Emissão de NFC-e para os pedidos via Focus NFe. Cobrado junto com a assinatura do seu plano, em uma única fatura.
                </p>
            </div>
            <div class="flex items-center gap-4">
                ...
                @else
                    <div class="text-right">
                        <span class="text-xl font-bold text-neutral-800 dark:text-neutral-100">
                            R$ {{ number_format($fiscalAddonAmount, 2, ',', '.') }}
                        </span>
                        <span class="text-sm text-neutral-500 dark:text-neutral-400">/mês</span>
                    </div>
                    <button wire:click="confirmFiscalActivation"
                            class="inline-flex items-center gap-1.5 bg-[#7A00A3] text-white text-sm font-medium px-4 py-2 rounded-lg hover:bg-[#5c0079] transition-colors">
                        Ativar módulo
                    </button>
                @endif
    ...
    {{-- Módulo Garçom (addon independente do plano, requer o módulo PDV ativo) --}}
    <div class="bg-white border rounded-xl shadow-sm p-6 space-y-4 dark:bg-zinc-800 dark:border-zinc-700">
        <div class="flex items-center justify-between flex-wrap gap-4">
            <div class="space-y-1">
                <h2 class="font-semibold text-neutral-700 text-sm uppercase tracking-wide dark:text-neutral-300">
                ...
                <p class="text-sm text-neutral-500 dark:text-neutral-400">
                    Acesso de garçom às mesas/comandas do PDV, sem operar caixa. Cobrado junto com a assinatura do seu plano, em uma única fatura. Requer o módulo PDV ativo.
                </p>
            </div>
            <div class="flex items-center gap-4">
                @if($waiterModuleEnabled)
                    <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-sm font-semibold bg-green-100 text-green-700 dark:bg-green-900/40 dark:text-green-400">
                        <span class="w-1.5 h-1.5 rounded-full bg-green-500"></span>
                        Ativo
                    </span>
                ...
                @elseif($pdvModuleEnabled)
                    <div class="text-right">
                        <span class="text-xl font-bold text-neutral-800 dark:text-neutral-100">
                            R$ {{ number_format($waiterAddonAmount, 2, ',', '.') }}
                        </span>
                        <span class="text-sm text-neutral-500 dark:text-neutral-400">/mês</span>
                    </div>
                    <button wire:click="confirmWaiterActivation"
                            class="inline-flex items-center gap-1.5 bg-[#7A00A3] text-white text-sm font-medium px-4 py-2 rounded-lg hover:bg-[#5c0079] transition-colors">
                        Ativar módulo
                    </button>
                @else
    ...
    {{-- Modal de confirmação de ativação do módulo PDV --}}
    <flux:modal name="confirm-pdv-activation" class="max-w-sm">
        <div class="space-y-5" x-data="{ termsAccepted: false }"
             x-init="$watch(() => $wire.confirmingPdvActivation, val => { if (val) termsAccepted = false; })">
            <div class="flex items-start gap-4">
                <div class="w-10 h-10 rounded-full bg-blue-100 dark:bg-blue-900/30 flex items-center justify-center shrink-0">
                    <span class="text-xl">🖥️</span>
                </div>
                <div>
                    <flux:heading size="lg">Ativar módulo PDV?</flux:heading>
                    <flux:subheading class="mt-1">
                        O valor do módulo (<strong>R$ {{ number_format($pdvAddonAmount, 2, ',', '.') }}/mês</strong>) será somado à sua assinatura, totalizando <strong>R$ {{ number_format($combinedMonthlyAmount, 2, ',', '.') }}/mês</strong> em uma única cobrança no cartão de crédito. Sua assinatura atual será recriada e o primeiro mês é cobrado imediatamente.
                    </flux:subheading>
                </div>
            </div>
            ...
            <div class="flex justify-end gap-3 pt-1">
                <flux:modal.close>
                    <flux:button variant="ghost" wire:click="cancelPdvActivation">Cancelar</flux:button>
                </flux:modal.close>
                <span :class="{ 'opacity-50 pointer-events-none cursor-not-allowed': !termsAccepted }">
                    <flux:button
                        x-on:click="$wire.pdvAcceptedTerms = termsAccepted; $wire.proceedToPdvCardModal()"
                        variant="primary">
    ...
    {{-- Modal de confirmação de cancelamento do módulo PDV --}}
    <flux:modal name="confirm-pdv-cancellation" class="max-w-sm">
        <div class="space-y-5">
            <div class="flex items-start gap-4">
                ...
                <div>
                    <flux:heading size="lg">Cancelar o módulo PDV?</flux:heading>
                    <flux:subheading class="mt-1">
                        O acesso ao terminal será removido e o valor será debitado da sua assinatura a partir do próximo vencimento.
                    </flux:subheading>
                </div>
            </div>
            ...
            <div class="flex justify-end gap-3 pt-1">
                <flux:modal.close>
                    <flux:button variant="ghost" wire:click="cancelPdvCancellation">Manter módulo</flux:button>
                </flux:modal.close>
                <flux:button wire:click="cancelPdvModule" variant="danger">Cancelar módulo</flux:button>
            </div>
        </div>
    </flux:modal>

    {{-- Modal de cartão de crédito para ativação do módulo PDV --}}
    <flux:modal name="pdv-module-card" class="max-w-lg">
        <div
            x-data="{
                maskCardNumber(e) {
                    let v = e.target.value.replace(/\D/g, '').substring(0, 16);
        ...
        >
            ...
            @if($pdvSuccess)
                <div class="flex flex-col items-center gap-3 py-6 text-center">
                    ...
                    <p class="font-semibold text-neutral-800 dark:text-neutral-100">Pagamento confirmado!</p>
                    <p class="text-sm text-neutral-500 dark:text-neutral-400">Módulo PDV ativado.</p>
                    <flux:modal.close>
            ...
            @else
                ...
                <button
                    ...
                    wire:loading.attr="disabled"
                    wire:loading.class="opacity-75 cursor-not-allowed"
                    @if($pdvProcessing) disabled @endif
                    class="w-full flex items-center justify-center gap-2 rounded-xl px-6 py-3 text-sm font-bold text-white shadow-sm transition-all duration-200 disabled:opacity-75"
                    style="background: linear-gradient(135deg, #5c0079, #7A00A3);"
                >
                    <span wire:loading.remove wire:target="activatePdvModule">
                        <svg class="h-4 w-4 inline mr-1" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M2.25 8.25h19.5M2.25 9h19.5m-16.5 5.25h6m-6 2.25h3m-3.75 3h15a2.25 2.25 0 0 0 2.25-2.25V6.75A2.25 2.25 0 0 0 19.5 4.5h-15a2.25 2.25 0 0 0-2.25 2.25v10.5A2.25 2.25 0 0 0 4.5 19.5Z" />
                        </svg>
                        Pagar e ativar módulo
                    </span>
                    <span wire:loading wire:target="activatePdvModule" class="flex items-center gap-2">
                        <svg class="h-4 w-4 animate-spin" fill="none" viewBox="0 0 24 24">
                            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"/>
                        </svg>
                        Processando...
    ...
    {{-- Modal de confirmação de ativação do módulo Fiscal --}}
    <flux:modal name="confirm-fiscal-activation" class="max-w-sm">
        <div class="space-y-5" x-data="{ termsAccepted: false }"
             x-init="$watch(() => $wire.confirmingFiscalActivation, val => { if (val) termsAccepted = false; })">
            <div class="flex items-start gap-4">
                <div class="w-10 h-10 rounded-full bg-blue-100 dark:bg-blue-900/30 flex items-center justify-center shrink-0">
                    <span class="text-xl">🧾</span>
                </div>
                <div>
                    <flux:heading size="lg">Ativar módulo Fiscal?</flux:heading>
                    <flux:subheading class="mt-1">
                        O valor do módulo (<strong>R$ {{ number_format($fiscalAddonAmount, 2, ',', '.') }}/mês</strong>) será somado à sua assinatura, totalizando <strong>R$ {{ number_format($combinedMonthlyAmount, 2, ',', '.') }}/mês</strong> em uma única cobrança no cartão de crédito. Sua assinatura atual será recriada e o primeiro mês é cobrado imediatamente.
                    </flux:subheading>
                </div>
            </div>
            <div class="border rounded-lg px-4 py-3 bg-neutral-50 dark:bg-zinc-700/50 dark:border-zinc-600">
            ...
            <div class="flex justify-end gap-3 pt-1">
                <flux:modal.close>
                    <flux:button variant="ghost" wire:click="cancelFiscalActivation">Cancelar</flux:button>
                </flux:modal.close>
                <span :class="{ 'opacity-50 pointer-events-none cursor-not-allowed': !termsAccepted }">
                    <flux:button
                        x-on:click="$wire.fiscalAcceptedTerms = termsAccepted; $wire.proceedToFiscalCardModal()"
                        variant="primary">
                        Continuar para pagamento
                    </flux:button>
                </span>
    ...
    {{-- Modal de confirmação de cancelamento do módulo Fiscal --}}
    <flux:modal name="confirm-fiscal-cancellation" class="max-w-sm">
        <div class="space-y-5">
            <div class="flex items-start gap-4">
                <div class="w-10 h-10 rounded-full bg-red-100 dark:bg-red-900/30 flex items-center justify-center shrink-0">
                    <svg class="w-5 h-5 text-red-600 dark:text-red-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div>
                    <flux:heading size="lg">Cancelar o módulo Fiscal?</flux:heading>
                    <flux:subheading class="mt-1">
                        A emissão de NFC-e será desabilitada e o valor será debitado da sua assinatura a partir do próximo vencimento.
                    </flux:subheading>
                </div>
            </div>
            ...
            <div class="flex justify-end gap-3 pt-1">
                <flux:modal.close>
                    <flux:button variant="ghost" wire:click="cancelFiscalCancellation">Manter módulo</flux:button>
                </flux:modal.close>
                <flux:button wire:click="cancelFiscalModule" variant="danger">Cancelar módulo</flux:button>
            </div>
        </div>
    </flux:modal>

    {{-- Modal de cartão de crédito para ativação do módulo Fiscal --}}
    <flux:modal name="fiscal-module-card" class="max-w-lg">
        <div
            x-data="{
                maskCardNumber(e) {
                    let v = e.target.value.replace(/\D/g, '').substring(0, 16);
        ...
        >
            ...
            @else
                ...
                <button
                    ...
                    wire:loading.attr="disabled"
                    wire:loading.class="opacity-75 cursor-not-allowed"
                    @if($fiscalProcessing) disabled @endif
                    class="w-full flex items-center justify-center gap-2 rounded-xl px-6 py-3 text-sm font-bold text-white shadow-sm transition-all duration-200 disabled:opacity-75"
                    style="background: linear-gradient(135deg, #5c0079, #7A00A3);"
                >
                    <span wire:loading.remove wire:target="activateFiscalModule">
                        <svg class="h-4 w-4 inline mr-1" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M2.25 8.25h19.5M2.25 9h19.5m-16.5 5.25h6m-6 2.25h3m-3.75 3h15a2.25 2.25 0 0 0 2.25-2.25V6.75A2.25 2.25 0 0 0 19.5 4.5h-15a2.25 2.25 0 0 0-2.25 2.25v10.5A2.25 2.25 0 0 0 4.5 19.5Z" />
                        </svg>
                        Pagar e ativar módulo
                    </span>
                    <span wire:loading wire:target="activateFiscalModule" class="flex items-center gap-2">
                        <svg class="h-4 w-4 animate-spin" fill="none" viewBox="0 0 24 24">
                            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"/>
                        </svg>
                        Processando...
    ...
    {{-- Modal de confirmação de ativação do módulo Garçom --}}
    <flux:modal name="confirm-waiter-activation" class="max-w-sm">
        <div class="space-y-5" x-data="{ termsAccepted: false }"
             x-init="$watch(() => $wire.confirmingWaiterActivation, val => { if (val) termsAccepted = false; })">
            <div class="flex items-start gap-4">
                <div class="w-10 h-10 rounded-full bg-blue-100 dark:bg-blue-900/30 flex items-center justify-center shrink-0">
                    <span class="text-xl">🙋</span>
                </div>
                <div>
                    <flux:heading size="lg">Ativar módulo Garçom?</flux:heading>
                    <flux:subheading class="mt-1">
                        O valor do módulo (<strong>R$ {{ number_format($waiterAddonAmount, 2, ',', '.') }}/mês</strong>) será somado à sua assinatura, totalizando <strong>R$ {{ number_format($combinedMonthlyAmount, 2, ',', '.') }}/mês</strong> em uma única cobrança no cartão de crédito. Sua assinatura atual será recriada e o primeiro mês é cobrado imediatamente.
                    </flux:subheading>
                </div>
            </div>
            <div class="border rounded-lg px-4 py-3 bg-neutral-50 dark:bg-zinc-700/50 dark:border-zinc-600">
            ...
            <div class="flex justify-end gap-3 pt-1">
                <flux:modal.close>
                    <flux:button variant="ghost" wire:click="cancelWaiterActivation">Cancelar</flux:button>
                </flux:modal.close>
                <span :class="{ 'opacity-50 pointer-events-none cursor-not-allowed': !termsAccepted }">
                    <flux:button
                        x-on:click="$wire.waiterAcceptedTerms = termsAccepted; $wire.proceedToWaiterCardModal()"
                        variant="primary">
                        Continuar para pagamento
                    </flux:button>
                </span>
    ...
    {{-- Modal de confirmação de cancelamento do módulo Garçom --}}
    <flux:modal name="confirm-waiter-cancellation" class="max-w-sm">
        <div class="space-y-5">
            <div class="flex items-start gap-4">
                <div class="w-10 h-10 rounded-full bg-red-100 dark:bg-red-900/30 flex items-center justify-center shrink-0">
                    <svg class="w-5 h-5 text-red-600 dark:text-red-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div>
                    <flux:heading size="lg">Cancelar o módulo Garçom?</flux:heading>
                    <flux:subheading class="mt-1">
                        O acesso de garçom às mesas/comandas será removido e o valor será debitado da sua assinatura a partir do próximo vencimento.
                    </flux:subheading>
                </div>
            </div>
            ...
            <div class="flex justify-end gap-3 pt-1">
                <flux:modal.close>
                    <flux:button variant="ghost" wire:click="cancelWaiterCancellation">Manter módulo</flux:button>
                </flux:modal.close>
                <flux:button wire:click="cancelWaiterModule" variant="danger">Cancelar módulo</flux:button>
            </div>
        </div>
    </flux:modal>

    {{-- Modal de cartão de crédito para ativação do módulo Garçom --}}
    <flux:modal name="waiter-module-card" class="max-w-lg">
        <div
            x-data="{
                maskCardNumber(e) {
                    let v = e.target.value.replace(/\D/g, '').substring(0, 16);
        ...
        >
            ...
            @if($waiterSuccess)
                <div class="flex flex-col items-center gap-3 py-6 text-center">
                    ...
                    <p class="font-semibold text-neutral-800 dark:text-neutral-100">Pagamento confirmado!</p>
                    <p class="text-sm text-neutral-500 dark:text-neutral-400">Módulo Garçom ativado.</p>
                    <flux:modal.close>
                        <flux:button variant="primary">Fechar</flux:button>
                    </flux:modal.close>
                </div>
            @else
                ...
                <button
                    wire:click="activateWaiterModule"
                    wire:loading.attr="disabled"
                    wire:loading.class="opacity-75 cursor-not-allowed"
                    @if($waiterProcessing) disabled @endif
                    class="w-full flex items-center justify-center gap-2 rounded-xl px-6 py-3 text-sm font-bold text-white shadow-sm transition-all duration-200 disabled:opacity-75"
                    style="background: linear-gradient(135deg, #5c0079, #7A00A3);"
                >
                    <span wire:loading.remove wire:target="activateWaiterModule">
                        <svg class="h-4 w-4 inline mr-1" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M2.25 8.25h19.5M2.25 9h19.5m-16.5 5.25h6m-6 2.25h3m-3.75 3h15a2.25 2.25 0 0 0 2.25-2.25V6.75A2.25 2.25 0 0 0 19.5 4.5h-15a2.25 2.25 0 0 0-2.25 2.25v10.5A2.25 2.25 0 0 0 4.5 19.5Z" />
                        </svg>
                        Pagar e ativar módulo
                    </span>
                    <span wire:loading wire:target="activateWaiterModule" class="flex items-center gap-2">
                        <svg class="h-4 w-4 animate-spin" fill="none" viewBox="0 0 24 24">
                            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"/>
                        </svg>
                        Processando...
