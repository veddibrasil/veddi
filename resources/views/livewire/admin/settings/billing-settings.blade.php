<div class="w-full space-y-6"
     x-init="
         ...
         $watch(() => $wire.confirmingPortalsActivation, val => val ? $flux.modal('confirm-portals-activation').show() : $flux.modal('confirm-portals-activation').close());
         $watch(() => $wire.confirmingPortalsCancellation, val => val ? $flux.modal('confirm-portals-cancellation').show() : $flux.modal('confirm-portals-cancellation').close());
     "
     ...
     x-on:open-portals-card-modal.window="$flux.modal('portals-module-card').show()">
    ...
    {{-- Módulo Portais (addon independente do plano) --}}
    <div class="bg-white border rounded-xl shadow-sm p-6 space-y-4 dark:bg-zinc-800 dark:border-zinc-700">
        <div class="flex items-center justify-between flex-wrap gap-4">
            <div class="space-y-1">
                <h2 class="font-semibold text-neutral-700 text-sm uppercase tracking-wide dark:text-neutral-300">
                ...
                <p class="text-sm text-neutral-500 dark:text-neutral-400">
                    Receba pedidos do iFood direto no seu painel. Cobrado junto com a assinatura do seu plano, em uma única fatura.
                </p>
            </div>
            <div class="flex items-center gap-4">
                @if($portalsModuleEnabled)
                    <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-sm font-semibold bg-green-100 text-green-700 dark:bg-green-900/40 dark:text-green-400">
                        <span class="w-1.5 h-1.5 rounded-full bg-green-500"></span>
                        Ativo
                    </span>
                    <a href="{{ route('admin.portals.index') }}" wire:navigate
                       class="text-sm text-amber-600 hover:text-amber-700 dark:text-amber-400 transition-colors">
                        Configurar
                    </a>
                    <button wire:click="confirmPortalsCancellation"
                            class="text-sm text-neutral-400 hover:text-red-500 dark:text-neutral-500 dark:hover:text-red-400 transition-colors">
                        Cancelar módulo
                    </button>
                @else
                    <div class="text-right">
                        <span class="text-xl font-bold text-neutral-800 dark:text-neutral-100">
                            R$ {{ number_format($portalsAddonAmount, 2, ',', '.') }}
                        </span>
                        <span class="text-sm text-neutral-500 dark:text-neutral-400">/mês</span>
                    </div>
                    <button wire:click="confirmPortalsActivation"
                            class="inline-flex items-center gap-1.5 bg-[#7A00A3] text-white text-sm font-medium px-4 py-2 rounded-lg hover:bg-[#5c0079] transition-colors">
                        Ativar módulo
                    </button>
                @endif
    ...
    {{-- Modal de confirmação de ativação do módulo Portais --}}
    <flux:modal name="confirm-portals-activation" class="max-w-sm">
        <div class="space-y-5" x-data="{ termsAccepted: false }"
             x-init="$watch(() => $wire.confirmingPortalsActivation, val => { if (val) termsAccepted = false; })">
            <div class="flex items-start gap-4">
                <div class="w-10 h-10 rounded-full bg-blue-100 dark:bg-blue-900/30 flex items-center justify-center shrink-0">
                    <span class="text-xl">🛵</span>
                </div>
                <div>
                    <flux:heading size="lg">Ativar módulo Portais?</flux:heading>
                    <flux:subheading class="mt-1">
                        O valor do módulo (<strong>R$ {{ number_format($portalsAddonAmount, 2, ',', '.') }}/mês</strong>) será somado à sua assinatura, totalizando <strong>R$ {{ number_format($portalsCombinedMonthlyAmount, 2, ',', '.') }}/mês</strong> em uma única cobrança no cartão de crédito. Sua assinatura atual será recriada e o primeiro mês é cobrado imediatamente.
                    </flux:subheading>
                </div>
            </div>
            <div class="border rounded-lg px-4 py-3 bg-neutral-50 dark:bg-zinc-700/50 dark:border-zinc-600">
            ...
            <div class="flex justify-end gap-3 pt-1">
                <flux:modal.close>
                    <flux:button variant="ghost" wire:click="cancelPortalsActivation">Cancelar</flux:button>
                </flux:modal.close>
                <span :class="{ 'opacity-50 pointer-events-none cursor-not-allowed': !termsAccepted }">
                    <flux:button
                        x-on:click="$wire.portalsAcceptedTerms = termsAccepted; $wire.proceedToPortalsCardModal()"
                        variant="primary">
                        Continuar para pagamento
                    </flux:button>
                </span>
    ...
    {{-- Modal de confirmação de cancelamento do módulo Portais --}}
    <flux:modal name="confirm-portals-cancellation" class="max-w-sm">
        <div class="space-y-5">
            <div class="flex items-start gap-4">
                <div class="w-10 h-10 rounded-full bg-red-100 dark:bg-red-900/30 flex items-center justify-center shrink-0">
                    <svg class="w-5 h-5 text-red-600 dark:text-red-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div>
                    <flux:heading size="lg">Cancelar o módulo Portais?</flux:heading>
                    <flux:subheading class="mt-1">
                        Pedidos do iFood deixarão de ser criados automaticamente e o valor será debitado da sua assinatura a partir do próximo vencimento.
                    </flux:subheading>
                </div>
            </div>
            ...
            @if($portalsError)
                <div class="flex items-start gap-3 rounded-xl border border-red-200 bg-red-50 dark:bg-red-900/20 dark:border-red-700/50 p-4 text-sm text-red-700 dark:text-red-400">
                    ...
                    <span>{{ $portalsError }}</span>
                </div>
            @endif

            <div class="flex justify-end gap-3 pt-1">
                <flux:modal.close>
                    <flux:button variant="ghost" wire:click="cancelPortalsCancellation">Manter módulo</flux:button>
                </flux:modal.close>
                <flux:button wire:click="cancelPortalsModule" variant="danger">Cancelar módulo</flux:button>
            </div>
        </div>
    </flux:modal>

    {{-- Modal de cartão de crédito para ativação do módulo Portais --}}
    <flux:modal name="portals-module-card" class="max-w-lg">
        <div
            x-data="{
                maskCardNumber(e) {
                    let v = e.target.value.replace(/\D/g, '').substring(0, 16);
        ...
        >
            ...
            @if($portalsSuccess)
                <div class="flex flex-col items-center gap-3 py-6 text-center">
                    ...
                    <p class="font-semibold text-neutral-800 dark:text-neutral-100">Pagamento confirmado!</p>
                    <p class="text-sm text-neutral-500 dark:text-neutral-400">Módulo Portais ativado.</p>
                    <flux:modal.close>
                        <flux:button variant="primary">Fechar</flux:button>
                    </flux:modal.close>
                </div>
            @else
                ...
                <button
                    wire:click="activatePortalsModule"
                    wire:loading.attr="disabled"
                    wire:loading.class="opacity-75 cursor-not-allowed"
                    @if($portalsProcessing) disabled @endif
                    class="w-full flex items-center justify-center gap-2 rounded-xl px-6 py-3 text-sm font-bold text-white shadow-sm transition-all duration-200 disabled:opacity-75"
                    style="background: linear-gradient(135deg, #5c0079, #7A00A3);"
                >
                    <span wire:loading.remove wire:target="activatePortalsModule">
                        <svg class="h-4 w-4 inline mr-1" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M2.25 8.25h19.5M2.25 9h19.5m-16.5 5.25h6m-6 2.25h3m-3.75 3h15a2.25 2.25 0 0 0 2.25-2.25V6.75A2.25 2.25 0 0 0 19.5 4.5h-15a2.25 2.25 0 0 0-2.25 2.25v10.5A2.25 2.25 0 0 0 4.5 19.5Z" />
                        </svg>
                        Pagar e ativar módulo
                    </span>
                    <span wire:loading wire:target="activatePortalsModule" class="flex items-center gap-2">
                        <svg class="h-4 w-4 animate-spin" fill="none" viewBox="0 0 24 24">
                            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"/>
                        </svg>
                        Processando...
