<div class="max-w-lg space-y-6">
    <div class="flex items-center gap-3">
        <a href="{{ route('admin.products.index') }}" class="text-neutral-400 hover:text-neutral-600">←</a>
    ...
    <div class="bg-white border rounded-xl shadow-sm p-6 space-y-4">
        ...
        <div>
            <label class="block text-sm font-medium text-neutral-700 mb-1">Imagem (opcional)</label>
            <input wire:model="image" type="file" accept="image/*"
                class="block w-full text-sm text-neutral-500 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:bg-amber-50 file:text-amber-700 hover:file:bg-amber-100" />
        ...
        <div>
            <label class="block text-sm font-medium text-neutral-700 mb-2">Disponível nas filiais</label>
            <div class="space-y-1">
                @foreach ($branches as $branch)
                    <label class="flex items-center gap-2 text-sm cursor-pointer">
                        <input type="checkbox" wire:model="selectedBranches" value="{{ $branch->id }}"
                            class="rounded border-neutral-300 text-amber-500 focus:ring-amber-400" />
                        {{ $branch->name }}
                    </label>
                @endforeach
            </div>
