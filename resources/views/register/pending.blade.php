<x-layouts::auth :title="'Aguardando pagamento'">
    ...
    <div class="flex flex-col items-center gap-6 text-center w-full max-w-md">
        ...
        @if(config('app.debug'))
            <form method="POST" action="{{ route('dev.simulate.asaas-payment') }}" class="w-full">
                @csrf
                <button type="submit"
                    class="w-full rounded-lg border border-dashed border-amber-400 bg-amber-50 dark:bg-amber-900/20 px-4 py-2 text-sm font-medium text-amber-700 dark:text-amber-400 hover:bg-amber-100 dark:hover:bg-amber-900/40 transition">
                    [DEV] Simular pagamento da taxa de ativação
                </button>
            </form>
        @endif
