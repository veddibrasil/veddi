...
@php
    $primary      = $company?->primary_color      ?? '#16A34A';
    $primaryDark  = $company?->primary_color_dark  ?? '#15803D';
    $primaryLight = $company?->primary_color_light ?? '#22C55E';
...
<div style="max-width: 600px; margin: 40px auto; background: #fff; border-radius: 10px; overflow: hidden; box-shadow: 0 2px 12px rgba(0,0,0,.10);">
    ...
    <div style="background: linear-gradient(135deg, {{ $primaryDark }} 0%, {{ $primary }} 60%, {{ $primaryLight }} 100%); padding: 32px 40px; text-align: center;">
        ...
        <p style="color: rgba(255,255,255,0.75); margin: 4px 0 0; font-size: 13px;">Pedido finalizado</p>
    </div>

    {{-- Body --}}
    <div style="padding: 32px 40px;">
        <h2 style="font-size: 20px; margin-top: 0; color: #111;">Seu pedido foi finalizado, {{ $customer->name }}!</h2>
        <p style="font-size: 15px; line-height: 1.6; color: #555;">
            Obrigado por comprar com a gente. Esperamos que tenha gostado!
        </p>
        ...
        <div style="background: #f9f9f9; border: 1px solid #e5e5e5; border-left: 4px solid {{ $primary }}; border-radius: 6px; padding: 20px 24px; margin: 24px 0;">
            ...
            <p style="margin: 0; font-size: 14px;"><span style="color: #888;">Status:</span> &nbsp;{{ $order->status_label }}</p>
        </div>
